	...
	f.Type().DefsFunc(func(g *jen.Group) {
		for _, t := range exportTypes {
			if _, ok := public[t]; !ok {
				...
				fmt.Printf("name collision for function %q. skipping export...")
			}
		}
	})
	...
	f.Const().DefsFunc(func(g *jen.Group) {
		for _, c := range exportConstants {
			if _, ok := public[c]; !ok {
				...
				fmt.Printf("name collision for constant %q. skipping export...")
			}
		}
	})
	...
	f.Var().DefsFunc(func(g *jen.Group) {
		for _, v := range exportVariables {
			if _, ok := public[v]; !ok {
				...
				fmt.Printf("name collision for variable %q. skipping export...")
			}
		}
	})
